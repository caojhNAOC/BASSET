...
static int get_data(float **outdata, int blocksperread,
                    ...
                    cudaStream_t main_stream, cudaStream_t sub_stream);
...
void adaptive_filter(float *data, float *data_gpu, int numpts, int numchan, int *BASSET_Ls, int BASSET_Ls_num, int BASSET_minL, int one_ms_len,
...
void multi_trigger(int start_index, int numpts, int one_ms_len, int work_length,
                   int BASSET_Ls_num, int *trigger_flag, float *SNRs_cpu);
void multi_trigger_else(int start_index, int numpts, int one_ms_len, int work_length,
                        float *data, int numchan, int bg_length, int *trigger_flag, float *autocorrelation_result_cpu, int min_BASSET_Ls);
void multi_range_search(int start_index, int numpts, int one_ms_len, int work_length,
                        float *data, int numchan, int bg_length, int *trigger_flag, int BASSET_minL, int min_BASSET_Ls);

extern void DownsampDataArray(float *currentdsdata_gpu, float *currentdata_gpu, int worklen, int bs, int nsub);
extern void float_dedisp_gpu(float *currentdsdata_gpu, float *lastdsdata_gpu, float *outdata_gpu, int worklen, int nsub, int numdms, int *offsets, cudaStream_t stream);
extern void float_dedisp_waterfall_gpu(float *currentdsdata_gpu, float *lastdsdata_gpu, float *outdata_gpu, int worklen, int nsub, int numdms, int *offsets, cudaStream_t stream);
extern void float_dedisp_waterfall_gpu2(float *currentdsdata_gpu, float *lastdsdata_gpu, float *outdata_gpu, int worklen, int nsub, int numdms, int *offsets, float *channel_means_gpu, float *channel_means_std_gpu, float *channel_means_mean_gpu, cudaStream_t stream);
extern void dsamp_in_time_gpu(int batchsize, int numchan, int dsamp_len, long long start_index, int one_ms_len, float *inputs, float *outputs, cudaStream_t stream);
extern void dsamp_in_time_gpu_bg(int batchsize, int numchan, int dsamp_len, long long start_index, int one_ms_len, float *inputs, float *outputs, int tmp, cudaStream_t stream);
extern void boxcar_two_prefix_gpu(int batchsize, int numchan, float *matrix, float *bg, float *sums, cudaStream_t stream);
extern void boxcar_two_suffix_gpu(int batchsize, int numchan, int BASSET_Ls_num, int *BASSET_Ls, float *sums, float *corr, cudaStream_t stream);
extern void get_SNR_gpu(int batchsize, float *corr, float *corr_bg, int numchan, int *BASSET_Ls, int *widths_gpu, int BASSET_Ls_num, float *SNRs_gpu, cudaStream_t stream);
extern void convolve_gpu(int batchsize, float *input, size_t input_size, float *output, size_t kernel_size, cudaStream_t stream);
extern void autocorrelation_gpu(int batchsize, float *input, size_t input_size, float *output, cudaStream_t stream);

float *oridata_gpu, *outdata_gpu, *data_corr_gpu;
float *currentdata_gpu, *lastdata_gpu, *currentdsdata_gpu, *lastdsdata_gpu;
int *offsets_gpu, *widths_gpu;
int gpu_worklen;
float *matrix_gpu, *bg_gpu, *sums_gpu, *corr_gpu, *SNRs_gpu;
float *matrix_boxcar_gpu, *autocorrelation_result_gpu;
float *matrix_cpu, *SNRs_cpu, *autocorrelation_result_cpu;
float *channel_means_gpu, *channel_means_std_gpu, *channel_means_mean_gpu;
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        omp_set_num_threads(num_procs * 4 / 5); // Explicitly turn off OpenMP
        printf("Using %d threads with OpenMP\n\n", num_procs * 4 / 5);
#endif
    }
    ...
    cudaMalloc((void **)&data_corr_gpu, sizeof(float) * gpu_worklen * cmd->numdms * cmd->nsub);
    cudaMalloc((void **)&offsets_gpu, sizeof(int) * cmd->numdms * cmd->nsub);
    cudaMalloc((void **)&channel_means_gpu, sizeof(float) * cmd->numdms * cmd->nsub);
    cudaMalloc((void **)&channel_means_std_gpu, sizeof(float) * cmd->numdms);
    cudaMalloc((void **)&channel_means_mean_gpu, sizeof(float) * cmd->numdms);
    ...
    cudaFreeAsync(oridata_gpu, main_stream);
    cudaFreeAsync(outdata_gpu, main_stream);
    cudaFreeAsync(data_corr_gpu, main_stream);
    cudaFreeAsync(offsets_gpu, main_stream);
    cudaFreeAsync(currentdata_gpu, sub_stream);
    cudaFreeAsync(lastdata_gpu, sub_stream);
    cudaFreeAsync(currentdsdata_gpu, sub_stream);
    cudaFreeAsync(lastdsdata_gpu, sub_stream);
    if (cmd->BASSET_LsP)
    {
        cudaFreeAsync(widths_gpu, main_stream);
        cudaFreeAsync(matrix_gpu, sub_stream);
        cudaFreeAsync(bg_gpu, sub_stream);
        cudaFreeAsync(sums_gpu, sub_stream);
        cudaFreeAsync(corr_gpu, sub_stream);
        cudaFreeAsync(SNRs_gpu, sub_stream);
        cudaFreeAsync(matrix_boxcar_gpu, sub_stream);
        cudaFreeAsync(autocorrelation_result_gpu, main_stream);
        cudaFreeAsync(channel_means_gpu, sub_stream);
        cudaFreeAsync(channel_means_std_gpu, sub_stream);
        cudaFreeAsync(channel_means_mean_gpu, sub_stream);
        free(matrix_cpu);
        free(SNRs_cpu);
        free(autocorrelation_result_cpu);
    }
                    ...
                    cudaStream_t main_stream, cudaStream_t sub_stream)
{
    ...
    if (!cmd->subP)
    {
        cudaMemcpyAsync(currentdsdata_gpu, currentdsdata, sizeof(float) * gpu_worklen * cmd->nsub, cudaMemcpyHostToDevice, main_stream); // GPU get data
        cudaMemcpyAsync(lastdsdata_gpu, lastdsdata, sizeof(float) * gpu_worklen * cmd->nsub, cudaMemcpyHostToDevice, main_stream);       // GPU get data

        if (!cmd->BASSET_LsP)
        {
            float_dedisp_gpu(currentdsdata_gpu, lastdsdata_gpu, outdata_gpu, gpu_worklen, cmd->nsub, cmd->numdms, offsets_gpu, main_stream);
            float *outdata_host = (float *)malloc(sizeof(float) * gpu_worklen * cmd->numdms);
            cudaMemcpyAsync(outdata_host, outdata_gpu, sizeof(float) * gpu_worklen * cmd->numdms, cudaMemcpyDeviceToHost, main_stream);
        ...
        {
            cudaMemsetAsync(channel_means_gpu, 0, sizeof(float) * cmd->nsub * cmd->numdms, sub_stream);
            data_corr_host = (float *)malloc(sizeof(float) * cmd->nsub * gpu_worklen * cmd->numdms);
            float_dedisp_waterfall_gpu2(currentdsdata_gpu, lastdsdata_gpu, data_corr_gpu, gpu_worklen, cmd->nsub, cmd->numdms, offsets_gpu, channel_means_gpu, channel_means_std_gpu, channel_means_mean_gpu, sub_stream);
            cudaMemcpyAsync(data_corr_host, data_corr_gpu, sizeof(float) * cmd->nsub * gpu_worklen * cmd->numdms, cudaMemcpyDeviceToHost, sub_stream);
        ...
        for (ii = 0; ii < cmd->numdms; ii++)
        {
            if (cmd->BASSET_LsP)
            {
                data_BASSET = (float *)malloc(sizeof(float) * cmd->nsub * dsworklen);
                cudaStreamSynchronize(sub_stream);
                memcpy(data_BASSET, data_corr_host + cmd->nsub * dsworklen * ii, sizeof(float) * cmd->nsub * dsworklen);

                adaptive_filter(data_BASSET, data_corr_gpu + cmd->nsub * dsworklen * ii, dsworklen, cmd->nsub, BASSET_Ls_list, *BASSET_Ls_num, cmd->BASSET_minL,
                          ...
                          int frequency_high, int *result_low, int *result_high)
{
    ...
    for (int ii = 0; ii < numchan - frequency_high; ++ii)
    {
        ...
        float SNR = (calculate_max(line, data_pulse_length) - calculate_mean(line_bg, data_bg_length)) / sum_of_squares_sqrt(stds, frequency_low, frequency_high + ii);
        SNRs[ii] = SNR;
    ...
    for (int ii = 0; ii < frequency_low; ++ii)
    {
        ...
        float SNR = (calculate_max(line, data_pulse_length) - calculate_mean(line_bg, data_bg_length)) / sum_of_squares_sqrt(stds, frequency_low - ii, frequency_high);
        SNRs[ii] = SNR;
...
int test_SNR(int BASSET_Ls_num, float *SNR_cpu, int b)
{
    float *SNRs = &SNR_cpu[b * BASSET_Ls_num];
                   ...
                   int BASSET_Ls_num, int *trigger_flag, float *SNRs_cpu)
{
    ...
    {
        trigger_flag[ii] = test_SNR(BASSET_Ls_num, SNRs_cpu, (ii - start_index));
    }
}
...
void multi_trigger_else(int start_index, int numpts, int one_ms_len, int work_length,
                        float *data, int numchan, int bg_length, int *trigger_flag, float *autocorrelation_result_cpu, int min_BASSET_Ls)
{
#pragma omp parallel for // 添加OpenMP指令
    for (int ii = start_index; ii < numpts / one_ms_len - 2 * work_length; ii++)
    {
        if (trigger_flag[ii])
        {
            float *autocorrelation_result = &autocorrelation_result_cpu[(ii - start_index) * (2 * numchan - 1)];
            int real_bandwidth = (int)fit_gaussian(autocorrelation_result, 2 * numchan - 1) * 2;
...
void multi_range_search(int start_index, int numpts, int one_ms_len, int work_length,
                        float *data, int numchan, int bg_length, int *trigger_flag, int BASSET_minL, int min_BASSET_Ls)
{
    int loop_num = (numpts / one_ms_len - 2 * work_length) - start_index;
    float **tmp_len, **ds_len, **autocorrelation_result;
    float **singal_array, **signal_array_boxcar, **channel_mean;
    tmp_len = (float **)malloc(loop_num * sizeof(float *));
    ds_len = (float **)malloc(loop_num * sizeof(float *));
    autocorrelation_result = (float **)malloc(loop_num * sizeof(float *));
    singal_array = (float **)malloc(loop_num * sizeof(float *));
    signal_array_boxcar = (float **)malloc(loop_num * sizeof(float *));
    channel_mean = (float **)malloc(loop_num * sizeof(float *));
    for (int i = 0; i < loop_num; ++i)
    {
        tmp_len[i] = (float *)malloc(bg_length * sizeof(float));
        ds_len[i] = (float *)malloc(numchan * sizeof(float));
        autocorrelation_result[i] = (float *)malloc((2 * numchan - 1) * sizeof(float));
        singal_array[i] = (float *)malloc(numchan * sizeof(float));
        signal_array_boxcar[i] = (float *)malloc(numchan * sizeof(float));
        channel_mean[i] = (float *)malloc(numchan * sizeof(float));
    }

    float **matrix_normalized_work;
    matrix_normalized_work = (float **)malloc(loop_num * (2 * work_length) * sizeof(float *));
    for (int i = 0; i < loop_num * 2 * work_length; ++i)
    {
...
#pragma omp parallel for // 添加OpenMP指令
    for (int ii = start_index; ii < numpts / one_ms_len - 2 * work_length; ii++)
    {
        int ff = ii - start_index;
        if (trigger_flag[ii])
        {
            for (int jj = 0; jj < bg_length; jj++)
            {
                dsamp_in_time(&data[numchan * (ii + jj) * one_ms_len], numchan, one_ms_len, ds_len[ff]);
                tmp_len[ff][jj] = calculate_sum(ds_len[ff], numchan);
            }
            long long max_bin = argmax(tmp_len[ff], bg_length) + ii;

            /* get channel_mean */
            dsamp_in_time(&data[numchan * (max_bin - (work_length + bg_length)) * one_ms_len], numchan, bg_length * one_ms_len, ds_len[ff]);
            for (int jj = 0; jj < numchan; jj++)
            {
                channel_mean[ff][jj] = ds_len[ff][jj] / bg_length;
            }

            /* get matrix_normalized_work */
            for (int jj = 0; jj < 2 * work_length; jj++)
            {
                dsamp_in_time(&data[numchan * (max_bin - work_length + jj) * one_ms_len], numchan, one_ms_len, ds_len[ff]);
                for (int mm = 0; mm < numchan; mm++)
                {
                    matrix_normalized_work[ff * 2 * work_length + jj][mm] = ds_len[ff][mm] - channel_mean[ff][mm];
                }
            }

            /* find right and left range */
            int right_range = find_right_range(&matrix_normalized_work[ff * 2 * work_length], work_length, numchan, BASSET_minL, min_BASSET_Ls);
            int left_range = find_left_range(&matrix_normalized_work[ff * 2 * work_length], work_length, numchan, BASSET_minL, min_BASSET_Ls);

            /* get real_bandwidth */
            calculate_matrix_sum(&matrix_normalized_work[ff * 2 * work_length], 2 * work_length, numchan, 0, work_length - left_range, work_length + right_range, singal_array[ff]);
            convolve(singal_array[ff], numchan, boxcar_kernel, BASSET_minL, signal_array_boxcar[ff]);
            autocorrelation(signal_array_boxcar[ff], numchan, autocorrelation_result[ff]);
            int real_bandwidth = (int)fit_gaussian(autocorrelation_result[ff], 2 * numchan - 1) * 2;
            if (numchan <= real_bandwidth || real_bandwidth <= min_BASSET_Ls / 1.5)
            {
                trigger_flag[ii] = 0;
            }
    ...
    for (int i = 0; i < loop_num; ++i)
    {
        free(tmp_len[i]);
        free(ds_len[i]);
        free(autocorrelation_result[i]);
        free(singal_array[i]);
        free(signal_array_boxcar[i]);
        free(channel_mean[i]);
    }
    free(tmp_len);
    free(ds_len);
                     ...
                     cudaStream_t main_stream, cudaStream_t sub_stream)
{
    ...
    float ds_len[numchan], bg[numchan], matrix_boxcar[numchan], channel_mean[numchan];
    ...
    float time;
    // char name[40];
    int trigger_flag[numpts / one_ms_len];
    int trigger_twice;
    int no_need_trigger;
    ...
    if (firsttime)
    {
        cudaMallocAsync(&widths_gpu, corr_Ls_num * sizeof(int), sub_stream);
        cudaMallocAsync(&matrix_gpu, sizeof(float) * (numpts / one_ms_len - 2 * work_length) * numchan, main_stream);
        cudaMallocAsync(&bg_gpu, sizeof(float) * (numpts / one_ms_len - 2 * work_length) * numchan, sub_stream);
        cudaMallocAsync(&sums_gpu, (numchan + 1) * sizeof(float) * (numpts / one_ms_len - 2 * work_length) * 2, sub_stream);
        cudaMallocAsync(&corr_gpu, corr_Ls_num * numchan * sizeof(float) * (numpts / one_ms_len - 2 * work_length) * 2, sub_stream);
        cudaMallocAsync(&SNRs_gpu, sizeof(float) * (numpts / one_ms_len - 2 * work_length) * corr_Ls_num, sub_stream);
        cudaMallocAsync(&matrix_boxcar_gpu, sizeof(float) * (numpts / one_ms_len - 2 * work_length) * numchan, sub_stream);

        cudaMemcpyAsync(widths_gpu, corr_Ls, corr_Ls_num * sizeof(int), cudaMemcpyHostToDevice, sub_stream);
        cudaMallocAsync(&autocorrelation_result_gpu, sizeof(float) * (numpts / one_ms_len - 2 * work_length) * (2 * numchan - 1), sub_stream);
        matrix_cpu = malloc(sizeof(float) * (numpts / one_ms_len - 2 * work_length) * numchan);
        SNRs_cpu = malloc(sizeof(float) * loop_num * corr_Ls_num);
        autocorrelation_result_cpu = malloc(sizeof(float) * (numpts / one_ms_len - 2 * work_length) * (2 * numchan - 1));

        firsttime = 0;
    }
    cudaDeviceSynchronize();

    dsamp_in_time_gpu(loop_num, numchan, bg_length * one_ms_len, start_index, one_ms_len, data_gpu, matrix_gpu, main_stream);
    dsamp_in_time_gpu_bg(loop_num, numchan, bg_length * one_ms_len, start_index, one_ms_len, data_gpu, bg_gpu, (work_length + bg_length), sub_stream);
    boxcar_two_prefix_gpu(loop_num, numchan, matrix_gpu, bg_gpu, sums_gpu, sub_stream);
    boxcar_two_suffix_gpu(loop_num, numchan, corr_Ls_num, widths_gpu, sums_gpu, corr_gpu, sub_stream);
    get_SNR_gpu(loop_num, corr_gpu, corr_gpu + corr_Ls_num * numchan * loop_num, numchan, corr_Ls, widths_gpu, corr_Ls_num, SNRs_gpu, sub_stream);

    float *SNRs_cpu = malloc(sizeof(float) * loop_num * corr_Ls_num);
    cudaMemcpyAsync(SNRs_cpu, SNRs_gpu, sizeof(float) * loop_num * corr_Ls_num, cudaMemcpyDeviceToHost, sub_stream);
    
    convolve_gpu(loop_num, matrix_gpu, numchan, matrix_boxcar_gpu, BASSET_minL, main_stream);
    autocorrelation_gpu(loop_num, matrix_boxcar_gpu, numchan, autocorrelation_result_gpu, main_stream);
    cudaMemcpyAsync(matrix_cpu, matrix_gpu, sizeof(float) * loop_num * numchan, cudaMemcpyDeviceToHost, main_stream);
    cudaMemcpyAsync(autocorrelation_result_cpu, autocorrelation_result_gpu, sizeof(float) * loop_num * (2 * numchan - 1), cudaMemcpyDeviceToHost, main_stream);
    cudaDeviceSynchronize();

    /*trigger first*/
    multi_trigger(start_index, numpts, one_ms_len, work_length, corr_Ls_num, trigger_flag, SNRs_cpu);
    multi_trigger_else(start_index, numpts, one_ms_len, work_length, data, numchan, bg_length, trigger_flag, autocorrelation_result_cpu, min_BASSET_Ls);
    ...
    for (ii = start_index; ii < numpts / one_ms_len - 2 * work_length; ii++)
    {
        ...
        else
        {
            ...
            boxcar_correlation(&matrix_cpu[(ii - start_index) * numchan], numchan, corr_Ls, corr_Ls_num, corr);
            ...
            else
            {
                if (!no_need_trigger)
                {
                    convolve(&matrix_cpu[(ii - start_index) * numchan], numchan, boxcar_kernel, BASSET_minL, matrix_boxcar);
