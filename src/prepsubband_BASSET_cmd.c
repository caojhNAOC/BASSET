...
static Cmdline cmd = {
  /* -BASSET_Ls: Lengths of the box-car function to match-filter pulses,
  *  with units in channels (at least 5 lengths, comma-separated string, no spaces) */
  /* BASSET_LsP = */ 0,
  /* BASSET_Ls = */ (char*)0,
  /* BASSET_LsC = */ 0,
...
showOptionValues(void)
{
  ...
  *  with units in channels (at least 5 lengths, comma-separated string, no spaces) */
  if( !cmd.BASSET_LsP ) {
    printf("-BASSET_Ls not found.\n");
  } else {
    printf("-BASSET_Ls found:\n");
    if( !cmd.BASSET_LsC ) {
      printf("  no values\n");
    } else {
      printf("  value = `%s'\n", cmd.BASSET_Ls);
...
parseCmdline(int argc, char **argv)
{
  ...
  for(i=1, cmd.argc=1; i<argc; i++) {
    ...
    if( 0==strcmp("-BASSET_Ls", argv[i]) ) {
      int keep = i;
      cmd.BASSET_LsP = 1;
      i = getStringOpt(argc, argv, i, &cmd.BASSET_Ls, 1);
      cmd.BASSET_LsC = i-keep;
      continue;
    }

    if( 0==strcmp("-BASSET_minL", argv[i]) ) {
      int keep = i;
      cmd.BASSET_minLP = 1;
      i = getIntOpt(argc, argv, i, &cmd.BASSET_minL, 1);
      cmd.BASSET_minLC = i-keep;
      checkIntHigher("-BASSET_minL", &cmd.BASSET_minL, cmd.BASSET_minLC, 1);
      continue;
    }
